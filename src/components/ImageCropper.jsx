  ...
  const handleMouseMove = useCallback((e) => {
    ...
    switch (dragging) {
      case 'move':
        ...
        break
      case 'nw':
        newCrop.x = Math.max(0, Math.min(startCrop.x + startCrop.width - minSize, startCrop.x + dx))
        newCrop.y = Math.max(0, Math.min(startCrop.y + startCrop.height - minSize, startCrop.y + dy))
      ...
      case 'ne':
        newCrop.y = Math.max(0, Math.min(startCrop.y + startCrop.height - minSize, startCrop.y + dy))
        newCrop.width = Math.max(minSize, Math.min(100 - startCrop.x, startCrop.width + dx))
        newCrop.height = startCrop.height - (newCrop.y - startCrop.y)
        break
      case 'e':
        newCrop.width = Math.max(minSize, Math.min(100 - startCrop.x, startCrop.width + dx))
      ...
      case 'se':
        newCrop.width = Math.max(minSize, Math.min(100 - startCrop.x, startCrop.width + dx))
        newCrop.height = Math.max(minSize, Math.min(100 - startCrop.y, startCrop.height + dy))
        break
      case 's':
        newCrop.height = Math.max(minSize, Math.min(100 - startCrop.y, startCrop.height + dy))
        break
        ...
        <div className="cropper-image-container">
          ...
          {imageLoaded && (
            ...
            >
              ...
              <div className="crop-edge top" onMouseDown={(e) => handleMouseDown(e, 'n')} />
              <div className="crop-edge bottom" onMouseDown={(e) => handleMouseDown(e, 's')} />
              <div className="crop-edge left" onMouseDown={(e) => handleMouseDown(e, 'w')} />
              <div className="crop-edge right" onMouseDown={(e) => handleMouseDown(e, 'e')} />
            </div>
          )}
        </div>
      </div>

      <p style={{ textAlign: 'center', marginTop: '1rem', color: 'rgba(255,255,255,0.5)' }}>
